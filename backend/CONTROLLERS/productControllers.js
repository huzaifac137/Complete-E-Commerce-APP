...
const getCart=async(req , res , next)=>
{
   ...
   try
   {
      allCartProducts = await PRODUCT.find({userOfProduct : userId});
   }
   ...
   catch(err)
   {
    ...
    return next(error);
     ...
     res.status(200).json({products: allCartProducts.map((item)=>item.toObject({getters:true})) });
}
...
const removeFromCart=async(req,res,next)=>
{
    ...
    try
    {
      product = await PRODUCT.findOne({_id : productId}).populate("userOfProduct");
    }
