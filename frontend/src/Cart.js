...
function Cart() {
    ...
    const {cartArray , Status2 , responseMessage2 , Status3 , responseMessage3 , totalPayment } = useSelector((store)=>store.cart);
    ...
    { (Status2==="error" || Status2==="idle" )&& <h2> {responseMessage2} </h2>  }
    ...
    {Status3==="loading" && <h2>REOMOVING FROM CART......</h2>}
     ...
     {Status3==="error" && <h2>{responseMessage3}</h2>}
        ...
        <div className="cart" key={item.id} >
         ...
         <div style={{backgroundColor:"grey" , padding:"2px" , borderRadius:"5px" , color:"white"}}> X{item.quantity}</div>
         <button className='btn' onClick={()=>handleRemoveFromCart(item.id)}>REMOVE </button> 
            ...
            <h2 style={{color:"green" , borderBottom : "2px solid grey" ,textAlign:"center" ,width:"50%" , margin:"10px auto"}}> 

            TOTAL PAYMENT : ${totalPayment.toFixed(2)} </h2> 

            <button className='btn' style={{margin:"30px"}} onClick={navigateToCheckout}> GO TO PAYMENT </button>
